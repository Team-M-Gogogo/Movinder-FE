...
export default function PaymentComponent() {
  ...
  const seats = useSelector((state) => {
    return state.movie.selectedSeats;
  });
  ...
  function handleSubmit() {
    var bookingRequestObj = {};
    bookingRequestObj["customerId"] = customerId;
    bookingRequestObj["movieSessionId"] = session.sessionId;
    bookingRequestObj["ticketRequestItems"] = tickets;
    bookingRequestObj["foodRequestItems"] = tickets;
    bookingRequestObj["seatingRequests"] = seatings;
    ...
    postBooking(bookingRequestObj)
      ...
      .catch((error) => {
        if (error.response.status === 400) {
          openNotification(
            "Oops",
            "Please start from home page. Reload of payment page not supported"
          );
        } else if (error.response.status === 409) {
          openNotification(
            "Oops",
            "Your selected seats has been taken. Please start from home page."
          );
        }
      });
  }
